...
wineModel = keras_model_sequential() %>%
  layer_dense(units = 64, activation = "relu",input_shape=(8), regularizer_l1(l = 0.01)) %>%
  layer_dense(units = 64, activation = "relu", regularizer_l1(l = 0.01)) %>%
  layer_dense(units = 64, activation = "relu", regularizer_l1(l = 0.01)) %>%
  layer_dense(units = 64, activation = "relu", regularizer_l1(l = 0.01)) %>%
...
xTr <- as.matrix(training[,1:8]) # need to convert to a matrix
xTest <- as.matrix(testing[,1:8])
